...
TEST_CASE("vector basic arithmetic", "[vector]") {
	...
	CHECK(complex_int{ 3,  -2 } * 1_i == mtl::__mtl_mul_by_i(complex_int{ 3, -2 }));
}
...
(_Tp,                                mtl::complex<_Up>,                  0), \
(mtl::complex<_Tp>,                  _Up,                                0), \
																			 \
...
(mtl::complex<_Tp>,                  mtl::vector3<_Up>,                  0), \
																			 \
(mtl::vector3<mtl::quaternion<_Tp>>, mtl::vector3<mtl::quaternion<_Up>>, 0), \
(mtl::vector3<mtl::quaternion<_Tp>>, mtl::vector3<mtl::complex<_Up>>,    0), \
(mtl::vector3<mtl::quaternion<_Tp>>, mtl::vector3<_Up>,                  0), \
(mtl::vector3<mtl::complex<_Tp>>,    mtl::vector3<mtl::quaternion<_Up>>, 0), \
...
TEMPLATE_TEST_CASE_SIG(__VA_ARGS__,                                                      \
					   ((typename T, typename U, int _ignore_this), T, U, _ignore_this), \
					   TYPE_LIST(int, int), TYPE_LIST(float, float), TYPE_LIST(int, float))

					
ARITHMETIC_INST_TEST("arithmetic instantiation test", "[vector][quaternion][complex]") {
	if ((0)) {
		(void)+T{};
		(void)-T{};
...
TEST_CASE("arithmetic matrix instantiation test", "[matrix][vector][quaternion][complex]") {
	using namespace mtl;
