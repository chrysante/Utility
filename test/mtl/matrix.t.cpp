...
TEMPLATE_TEST_CASE_SIG(__VA_ARGS__, \
					   ((typename T, std::size_t Rows, std::size_t Columns, mtl::vector_options Options), T, Rows, Columns, Options), \
					   TYPE_LIST(mtl::vector_options{}), TYPE_LIST(mtl::vector_options{}.packed(true)) \
					   )
...
namespace {
	template <typename T>
	struct OtherType { using type = int; };
	template <>
	struct OtherType<int> { using type = float; };
	template <mtl::vector_options O>
	struct OtherOptions { auto constexpr static value = O.packed(!O.packed()); };
}
...
MATRIX_TEST_CASE("matrix functional constructor", "[matrix]") {
	...
	SECTION("4") {
		...
		RowType rows[4] = {
			RowType([&](int i) { return values[ 0 + i]; }),
...
MATRIX_TEST_CASE("matrix conversion", "[matrix]") {
	...
	mtl::matrix<T, Rows, Columns, Options> const m([&](int i) { return values[i]; });
...
TEST_CASE("matrix::swizzle", "[matrix]") {
	...
	SECTION("row_swizzle") {
		CHECK(A.row_swizzle(2, 1, 0, 2) == int4x4{
			9, 0, 1, 2,
			5, 6, 7, 8,
			1, 2, 3, 4,
			9, 0, 1, 2
		});
		CHECK(A.row_swizzle(2, 1) == int2x4{
			9, 0, 1, 2,
			5, 6, 7, 8
		});
	}
	...
	SECTION("column_swizzle") {
		CHECK(A.column_swizzle(2, 3, 1) == int3x3{
			3, 4, 2,
			7, 8, 6,
			1, 2, 0
		});
		
		CHECK(A.column_swizzle(2, 3) == int3x2{
			3, 4,
			7, 8,
			1, 2
		});
