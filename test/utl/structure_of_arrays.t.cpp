...
UTL_SOA_TYPE(Particle, (float, position), (int, id), (double, color),
             (int, size), (LifetimeCounter, counter),
...
template <typename A>
static void testPushBack(utl::structure_of_arrays<Particle, A> const& s,
                         std::size_t size) {
    CHECK(s.size() == size);
...
TEST_CASE("structure_of_arrays::view") {
    utl::structure_of_arrays<Particle> const s = {
        Particle{ .position = 1, .id = 2, .color = 3, .size = 4 }, Particle{},
        Particle{}
    };
    CHECK(s.size() == 3);
    CHECK(s.capacity() >= 3);
...
TEST_CASE("structure_of_arrays::iterator") {
    ...
    for (int i = 0; i < size; ++i) {
        s.push_back(Particle{ .position = (rand() % 1024) / 1024.0f,
                              .id = rand() % 1024 });
    }
    SECTION("Arithmetic") {
...
TEST_CASE("std::sort(structure_of_arrays)") {
    ...
    for (int i = 0;
         auto [position, id] :
         s.view<Particle::members::position, Particle::members::id>())
    {
        CHECK(id == i);
...
TEST_CASE("structure_of_arrays::insert [range]") {
    ...
    REQUIRE(s.size() == 3);
    Particle p[3] = { { .id = -1, .counter{ false } },
                      { .id = -2, .counter{ false } },
                      { .id = -3, .counter{ false } } };
    SECTION("insert front") {
        s.insert(0, p, p + 3);
...
TEST_CASE("structure_of_arrays::insert [range2]") {
    ...
    Particle p[5] = { { .id = 0, .counter{ false } },
                      { .id = 1, .counter{ false } },
                      { .id = 2, .counter{ false } },
                      { .id = 3, .counter{ false } },
                      { .id = 4, .counter{ false } } };
...
TEST_CASE("structure_of_arrays::swap") {
    utl::structure_of_arrays<Particle> s = { Particle{ .id = 0 },
                                             Particle{ .id = 1 },
                                             Particle{ .id = 2 } };
    utl::structure_of_arrays<Particle> t = { Particle{ .id = 0 },
                                             Particle{ .id = -1 } };
    s.swap(t);
    CHECK(t.size() == 3);
    for (int i = 0; i < 3; ++i) {
...
TEST_CASE("structure_of_arrays iterate") {
    ...
    SECTION("iterate subset view") {
        for (int i = 0;
             auto [position, id] : s.view<Particle::members::position const,
                                          Particle::members::id const>())
        {
            CHECK(id == i);
            ++i;
        }
...
TEST_CASE("structure_of_arrays with STL algorithms") {
    ...
    std::generate(s.begin(), s.end(), [i = 0]() mutable {
        auto const result =
            Particle{ .position = static_cast<float>(9 - i), .id = 9 - i };
        ++i;
        return result;
    });
    for (int i = 0; i < 10; ++i) {
