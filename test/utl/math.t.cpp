...
static auto makeTestData(std::size_t count,
                         std::invocable<std::mt19937_64&> auto f) {
...
TEMPLATE_TEST_CASE("log2", "[math]", int, long, unsigned int, unsigned long) {
    ...
    auto const data =
        makeTestData(512,
                     [dist = std::uniform_int_distribution<TestType>(0, digits -
                                                                            1)](
                         auto& rng) mutable { return dist(rng); });
