...
TEST_CASE("utl::function local function rvalue reference argument", "[function]") {
	using namespace utl;
	...
	function<void(X&&)> f = [](X&& x) mutable { X const y = std::move(x); (void)y; };
...
//static_assert(sizeof(utl::function<void()>) == sizeof(void*) * UTL_FUNCTION_TARGET_SIZE_IN_WORDS);
...
namespace {
	...
	struct OperationCounter {
		...
		OperationCounter& operator=(OperationCounter const&) { ++copyAssignmentCount; return *this; }
		OperationCounter& operator=(OperationCounter&&) noexcept { ++moveAssignmentCount; return *this; }
	};
...
namespace {
	...
	U testFunctionAdd(T a, U& b) { return static_cast<U>(a + b); }
	long testFunctionMultiply(long a, int& b) { return a * b; }
}
...
TEMPLATE_TEST_CASE("utl::function function operation count", "[utl::function]",
				   Big, Small) {
	{
		...
		function<int()> f = [x = OperationCounter(), y = TestType{}]{ return 3; };
...
TEMPLATE_TEST_CASE("utl::unique_function function operation count", "[utl::unique_function]", Big, Small) {
	{
		using namespace utl;
		OperationCounter::resetCounters();
		
		unique_function<int()> f = [x = OperationCounter(), y = TestType{}]{ return 3; };
