...
TEST_CASE("topsort", "[graph]") {
    ...
    utl::topsort(indices.begin(), indices.end(),
                 [&](std::size_t index) { return vertices[index].successors; });
    /// For each index \p i all indices \p j coming afterwards in the sorted
    /// list must not be downstream of \p i
    ...
    /// For each pair of indices \p (i,j) \p j must be downstream of \p i only
    /// if \p j appears earlier in the sorted list.
    for (std::size_t i = 0; i < indices.size(); ++i) {
