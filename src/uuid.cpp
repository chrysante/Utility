...
uuid uuid::from_string(std::string&& str) {
    ...
    return std::bit_cast<uuid>(ints);
}

std::string uuid::to_string() const {
    auto const data =
        std::bit_cast<std::array<std::uint32_t, 4>>(this->value());
