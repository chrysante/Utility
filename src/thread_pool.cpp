...
utl::thread_pool::thread_pool(): thread_pool(std::thread::hardware_concurrency()) {}
...
void utl::thread_pool::cancel_current_tasks(bool wait_for_current) {
    std::unique_lock lock(m_mutex);
    std::for_each(m_items.begin(), m_items.end(), [](dispatch_item& item) {
        item.run_cancel_handler();
    });
    m_items.clear();
    if (wait_for_current) {
...
void utl::thread_pool::wait_for_current_tasks(std::unique_lock<std::mutex> lock) {
    m_control_cv.wait(lock, [this]{
        return m_items.empty() && all_idle();
    });
}
