...
struct variant_alternative<I, variant<T...>> {
    using type = typename type_sequence<T...>::template at<I>;
};
...
template <typename Var>
inline constexpr bool __is_variant =
    __is_variant_impl<std::decay_t<Var>>::value;

template <typename... Vars>
struct __variant_product_size:
    std::integral_constant<std::size_t,
                           __variant_product_size_impl<Vars...>()> {};

template <typename... Vars>
struct __variant_product_size<type_sequence<Vars...>>:
    __variant_product_size<std::decay_t<Vars>...> {};
...
template <typename... T, std::size_t... I>
struct __index_selector_impl<type_sequence<T...>, std::index_sequence<I...>>:
    __index_selector_overload<T, I>... {
    using __index_selector_overload<T, I>::__call...;
};

template <typename... T>
struct __index_selector:
    __index_selector_impl<type_sequence<T...>, std::index_sequence_for<T...>> {
};
...
//
// template <typename T>
// concept __trivially_copy_constructible =
...
//
// template <typename T>
// concept __trivially_move_constructible =
...
concept __has_get_by_type =
    Var::template __contains<T> && (Var::template __occurence_count<T> == 1);
...
template <bool DeduceReturnType, typename R, typename Visitor,
          ...
          typename FlatI =
              std::make_index_sequence<__variant_product_size<Variants>::value>>
...
template <bool DeduceReturnType, typename R, typename Visitor,
          ...
          std::size_t... FlatI> // Indices 0..<N into the flattened list of all
                                // combinations types in the variants
struct __variant_visit<DeduceReturnType, R, Visitor, type_sequence<Variants...>,
                       type_sequence<SizeT...>, std::index_sequence<I...>,
                       std::index_sequence<FlatI...>>:
    __farray_base<variant_size_v<std::remove_reference_t<Variants>>...> {
    using __base =
        __farray_base<variant_size_v<std::remove_reference_t<Variants>>...>;
    ...
    using __variant_at_index =
        typename type_sequence<Variants...>::template at<J>;

    static constexpr std::size_t __variant_count = sizeof...(Variants);
    static constexpr std::array<std::size_t, __variant_count>
        __variant_sizes = {
            variant_size_v<std::remove_reference_t<Variants>>...
        };

    template <std::size_t FlatIndex>
    using __deduced_return_type_at =
        std::invoke_result_t<Visitor,
                             decltype(get<__expand_index(FlatIndex)[I]>(
                                 std::declval<Variants&&>()))...>;

    static constexpr bool __all_are_references =
        __all<std::is_reference<__deduced_return_type_at<FlatI>>...>;
    static constexpr bool __none_is_reference =
        !__any<std::is_reference<__deduced_return_type_at<FlatI>>...>;

    static_assert(__all_are_references || __none_is_reference,
                  "Either all cases or none must return references. This "
                  "prevents accidental unintended copies or dangling "
                  "references to local stack memory.");
    ...
    static constexpr bool __all_return_types_equal =
        type_sequence<__deduced_return_type_at<FlatI>...>::all_equal;

    using __deduced_return_type =
        std::conditional_t<__all_return_types_equal,
                           __deduced_return_type_at<0>, __common_return_type>;

    using __return_type =
        std::conditional_t<DeduceReturnType, __deduced_return_type, R>;

    static constexpr __return_type visit(Visitor&& visitor,
                                         Variants&&... vars) {
        ...
        constexpr std::array<visit_fn, flat_array_size> function = {
            ...
            if constexpr (std::same_as<__return_type, void>) {
                std::invoke(std::forward<Visitor>(vis),
                            get<index[I]>(std::forward<Variants>(vars))...);
            }
            else {
        ...
        return function[flat_index](std::forward<Visitor>(visitor),
                                    std::forward<Variants>(vars)...);
    }
};
...
constexpr decltype(auto) visit(Visitor&& vis, Variants&&... vars) {
    return __variant_visit<
        true, void /* ignored */, Visitor,
        type_sequence<Variants&&...>>::visit(std::forward<Visitor>(vis),
                                             std::forward<Variants>(vars)...);
}
...
constexpr R visit(Visitor&& vis, Variants&&... vars) {
    return __variant_visit<false, R, Visitor, type_sequence<Variants&&...>>::
        visit(std::forward<Visitor>(vis), std::forward<Variants>(vars)...);
}

template <typename... T>
constexpr variant<T...>& __variant_cast(variant<T...>& v) {
    return v;
}
template <typename... T>
constexpr variant<T...> const& __variant_cast(variant<T...> const& v) {
    return v;
}
template <typename... T>
constexpr variant<T...>&& __variant_cast(variant<T...>&& v) {
...
public:
    ...
    template <typename T>
    static constexpr bool __occurence_count =
        __type_list::template occurence_count<T>;
    ...
    constexpr variant() noexcept(
        std::is_nothrow_default_constructible_v<__type_at_index<0>>)
    ...
    constexpr variant(variant const& rhs) noexcept(
        __all<std::is_nothrow_copy_constructible<Types>...>)
    requires __all<std::is_copy_constructible<Types>...>
    {
    ...
    constexpr variant(variant&& rhs) noexcept(
        __all<std::is_nothrow_move_constructible<Types>...>)
    requires __all<std::is_move_constructible<Types>...>
    {
    ...
    // (4)
    template <typename T,
              std::size_t Index = decltype(__index_selector<Types...>::__call(
                  std::declval<T>()))::value>
    ...
    requires __contains<T> &&
             (__occurence_count<T> == 1) && std::constructible_from<T, Args...>
    ...
    requires(I < __count) &&
            std::constructible_from<__type_at_index<I>, Args...>
    ...
    requires(I < __count) &&
            std::constructible_from<__type_at_index<I>,
    ...
    // Destructor
    //    ~variant() requires __all<std::is_trivially_destructible<Types>...> =
    //    default;

    constexpr ~variant() { __destroy(); }
    ...
    constexpr std::size_t index() const noexcept { return __index; }

    template <std::size_t I>
    requires(I < __count)
    constexpr bool is() const {
        return holds_alternative<I>(*this);
    }

    template <typename T>
    requires __contains<T> && (__occurence_count<T> == 1)
    constexpr bool is() const {
        return is<__index_of<T>>();
    }
    ...
    constexpr auto& emplace(std::initializer_list<U> il, Args&&... args) {
        emplace<I, std::initializer_list<U>, Args...>(il, std::forward<Args>(
                                                              args)...);
    }
    ...
    template <std::size_t I>
    requires(I < __count)
    variant_alternative<I, variant> const& get() const& {
        return utl::get<I>(*this);
    }

    template <std::size_t I>
    requires(I < __count)
    variant_alternative<I, variant>& get() & {
        return utl::get<I>(*this);
    }
    ...
    requires(I < __count)
    variant_alternative<I, variant> const&& get() const&& {
        return utl::get<I>(std::move(*this));
    }
    ...
    template <typename T>
    requires __has_get_by_type<T, variant>
    T const& get() const& {
        return utl::get<T>(*this);
    }
    ...
    requires __has_get_by_type<T, variant>
    T const&& get() const&& {
        return utl::get<T>(std::move(*this));
    }
    ...
    requires have_common_base<Base, Types...>::value &&
             std::same_as<std::remove_cvref_t<Base>, Base>
    constexpr Base const& as_base() const& {
        return __as_base<Base const&>(*this);
    }

    template <typename Base>
    requires have_common_base<Base, Types...>::value &&
             std::same_as<std::remove_cvref_t<Base>, Base>
    constexpr Base& as_base() & {
        return __as_base<Base&>(*this);
    }
    ...
    requires have_common_base<Base, Types...>::value &&
             std::same_as<std::remove_cvref_t<Base>, Base>
    constexpr Base const&& as_base() const&& {
        return __as_base<Base const&&>(std::move(*this));
    }
    ...
    requires have_common_base<Base, Types...>::value &&
             std::same_as<std::remove_cvref_t<Base>, Base>
    constexpr Base&& as_base() && {
        return __as_base<Base&&>(std::move(*this));
    }
    ...
    constexpr void __copy_or_move_construct(V&& rhs) {
        __visit_with_index(rhs.__index,
                           [&]<std::size_t I>(
                               std::integral_constant<std::size_t, I>) {
    ...
    static constexpr Base __virtual_as_base(Self&& self) {
        ...
        return
            ...
            return reinterpret_cast<Base>(
                *(reinterpret_cast<
                      copy_cv_t<std::remove_reference_t<Base>, char>*>(
                      &self.__data) +
...
#if UTL_HAS_COMPILE_TIME_BASE_OFFSET
        constexpr bool all_offsets_equal =
    ...
    template <typename F>
    static constexpr decltype(auto) __visit_with_index(std::size_t index,
                                                       F&& f) {
        return
            [&]<std::size_t... I>(std::index_sequence<I...>) -> decltype(auto) {
            using return_type = std::common_type_t<std::invoke_result_t<
...
public:
    ...
    constexpr Base* operator->() {
        return const_cast<Base*>(
            static_cast<cbvariant const*>(this)->operator->());
    }
    constexpr Base const* operator->() const {
        return &this->template as_base<Base>();
    }
