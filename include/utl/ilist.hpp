...
protected:
    __ilist_node_base(__ilist_node_base* prev = nullptr,
                      __ilist_node_base* next = nullptr):
        __prev(prev), __next(next) {}

    /// Copy operations are no-ops, as we don't want to propagate sibling
    /// relationship with copying.
...
private:
    ...
    /// Instead of copy construction and assignment we provide a special
    /// function to be used by ilist to copy relationship information.
...
public:
    ...
    Derived const* prev() const {
        return static_cast<Derived const*>(this->__prev);
    }
    Derived* next() { return static_cast<Derived*>(this->__next); }
    Derived const* next() const {
        return static_cast<Derived const*>(this->__next);
    }
};

template <typename Derived, bool AllowSetSiblings = false>
class ilist_node /* AllowSetSiblings = false */:
    public __ilist_node_base2<Derived> {};

template <typename Derived>
class ilist_node<Derived, /* AllowSetSiblings = */ true>:
    public __ilist_node_base2<Derived> {
public:
...
public:
    ...
    static constexpr bool __alloc_always_eq =
    ...
    private:
        ...
        requires std::is_const_v<N>
        {
            ...
            return __iterator_impl<std::remove_const_t<N>>(
                const_cast<mut_node_base*>(__node));
        }

        explicit __iterator_impl(__node_base_ptr node): __node(node) {}
    ...
    public:
        ...
        bool operator==(__iterator_impl<std::add_const_t<N>> const& rhs) const
        requires(!std::is_const_v<N>)
        {
            return __node == rhs.__node;
        }
        bool
        operator==(__iterator_impl<std::remove_const_t<N>> const& rhs) const
        requires std::is_const_v<N>
        {
            return __node == rhs.__node;
        }
    ...
    // (2)
    explicit ilist(allocator_type const& alloc):
        __allocator_(alloc), __sentinel_(__sentinel(), __sentinel()) {}

    // (3)
    explicit ilist(size_type count, value_type const& value,
    ...
    requires std::is_default_constructible_v<value_type>
        : ilist(alloc) {
        __insert_impl(begin(), count,
                      [this] { return __construct(__allocate()); });
    }
    ...
    requires std::is_default_constructible_v<allocator_type>
        :
        ilist(rhs, __alloc_traits::select_on_container_copy_construction(
                       rhs.__allocator_)) {}
    ...
    ilist(ilist&& rhs, allocator_type const& alloc) noexcept: ilist(alloc) {
        ...
        /// Use `if constexpr` here to prevent the path using `__insert_impl()`
        /// from compiling when not needed.
    ...
    // (10)
    ilist(std::initializer_list<T> init_list,
          allocator_type const& alloc = allocator_type()):
        ilist(alloc) {
    ...
    ilist& operator=(ilist const& rhs) {
        if constexpr (__alloc_traits::propagate_on_container_copy_assignment::
                          value)
        {
    ...
    ilist& operator=(ilist&& rhs) noexcept {
        constexpr bool alloc_propagate =
    ...
    void __move_assign_slow(ilist& rhs) {
        ...
        // Assign element-wise, as we must reallocate because unequal
        // allocators.
        __assign_element_wise(std::move_iterator(rhs.begin()),
    ...
    reference front() {
        return const_cast<reference>(static_cast<ilist const*>(this)->front());
    }
    ...
    reference back() {
        return const_cast<reference>(static_cast<ilist const*>(this)->back());
    }
    ...
    const_iterator begin() const {
        return const_iterator(__sentinel()->__next);
    }
    ...
    iterator insert(const_iterator pos, std::initializer_list<T> init_list) {
        return __insert_impl(pos, init_list.size(),
                             [this, itr = init_list.begin()]() mutable {
    ...
    [[nodiscard]] value_type* extract(const_iterator cpos) {
    ...
    /// \overload
    [[nodiscard]] value_type* extract(value_type const* element) {
    ...
    // (a)
    void push_back(value_type* node) { insert(end(), node); }
    ...
    // (a)
    void push_front(value_type* node) { insert(begin(), node); }
    ...
    /// Insert content of \p rhs into `this` before \p pos. Behaviour is
    /// undefined if `this == &rhs`.
    ...
    /// Insert range `[begin, end)` into `this` before \p pos. Behaviour is
    /// undefined if inserted range is part of `this`.
    ...
    void __assign_element_wise_f(std::invocable auto&& insert_while,
                                 auto&& get_elems) {
        iterator itr = begin();
        for (;; ++itr) {
            if (itr == end()) {
            ...
            /// We can safely assign because sibling pointers don't propagate on
            /// assignment.
            *itr = get_elems();
        }
    this_smaller:
        __insert_impl(end(), insert_while,
                      [&] { return __construct(__allocate(), get_elems()); });
        return;
    rhs_smaller:
        erase(itr, end());
        return;
    ...
    void __assign_element_wise(auto rhs_itr, auto rhs_end) {
        __assign_element_wise_f([&] { return rhs_itr != rhs_end; },
                                [&]() -> decltype(auto) { return *rhs_itr++; });
    }
    ...
    void __swap_impl(ilist& rhs) noexcept {
        if constexpr (SwapAllocs || __alloc_always_eq) {
            ...
            if (__allocator_ == rhs.__allocator_) {
                ...
                __assign_element_wise(iterator(rhs_copy.__next),
                                      iterator(&rhs_copy));
            }
        }
    }
    ...
    void __fast_swap(ilist& rhs) {
        ...
        __sentinel_type rhs_copy;
        __move_assign_pointer_swap_s(rhs_copy, rhs.begin(), rhs.end());
        __move_assign_pointer_swap_s(rhs.__sentinel_, this->begin(),
                                     this->end());
        __move_assign_pointer_swap_s(this->__sentinel_,
                                     iterator(rhs_copy.__next),
                                     iterator(&rhs_copy));
    }

    static void __move_assign_pointer_swap_s(__sentinel_type& sent,
    ...
    iterator __insert_impl(const_iterator cpos,
                           std::invocable auto&& insert_while,
    ...
    __sentinel_type* __sentinel() {
        return const_cast<__sentinel_type*>(
            static_cast<ilist const*>(this)->__sentinel());
    }
    ...
    void __deallocate(pointer p) {
        __alloc_traits::deallocate(__allocator_, p, 1);
    }
    ...
    void __destroy(pointer p) { __alloc_traits::destroy(__allocator_, p); }
...
operator<<(std::basic_ostream<CharT, Traits>& ostream, ilist<T, A> const& l) {
    ostream << "[";
