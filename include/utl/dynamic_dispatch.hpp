...
template <typename... T>
struct __is_dispatch_arg_tuple<__dispatch_arg_tuple<T...> const>:
    std::true_type {};
...
template <typename... T>
__dispatch_arg(__dispatch_arg_tuple<T...>, std::size_t)
    -> __dispatch_arg<__dispatch_arg_tuple<T...>>;
template <typename... T>
struct __dispatch_arg<__dispatch_arg_tuple<T...>> {
    __dispatch_arg(__dispatch_arg_tuple<T...> x, std::size_t i):
        x(x), index(i) {}
...
struct __dispatch_arg<std::integral_constant<std::size_t, N>> {
    __dispatch_arg(std::integral_constant<std::size_t, N> x, std::size_t i):
        x(x), index(i) {}
...
struct __is_integral_constant<std::integral_constant<T, Value>>:
    std::true_type {};
template <typename T, T Value>
struct __is_integral_constant<std::integral_constant<T, Value> const>:
    std::true_type {};

template <typename... T>
void __dispatch_one_impl(auto&& selector, __dispatch_arg_tuple<T...> const& t,
                         std::size_t index, auto&&... args) {
    ...
    UTL_WITH_INDEX_SEQUENCE((I, size), {
        ...
        (void)(... ||
               (I == index ?
                    (invoke(std::integral_constant<std::size_t, I>{}), true) :
                    false));
    });
}

template <typename Selector, typename SizeConstant>
requires requires(Selector&& s, SizeConstant c) { s(c); } &&
         (__is_integral_constant<SizeConstant>::value)
void __dispatch_one_impl(Selector&& selector, SizeConstant size_constant,
                         std::size_t index, auto&&... args) {
    ...
    UTL_WITH_INDEX_SEQUENCE((I, size), {
        auto invoke =
            [&]<std::size_t J>(std::integral_constant<std::size_t, J>) {
            selector(std::integral_constant<std::size_t, J>{},
                     UTL_FORWARD(args)...);
        };
...
void __dispatch_impl(std::integral_constant<std::size_t, Size - 1>,
                     auto&& selector,
                     __dispatch_arg_tuple<Args...> const& argTuple,
                     auto&&... x) {
...
void __dispatch_impl(std::integral_constant<std::size_t, Index>,
                     auto&& selector,
                     __dispatch_arg_tuple<Args...> const& argTuple,
                     auto&&... x) {
    static_assert(Index < Size - 1);
    __dispatch_one_impl([&](auto toUnpack, auto... x) {
        __dispatch_impl<Size>(std::integral_constant<std::size_t, Index + 1>{},
                              selector, argTuple, x..., toUnpack);
    }, get<Index>(argTuple).x, get<Index>(argTuple).index, x...);
}
...
auto dispatch_arg(std::size_t index, T&&... args) {
    __utl_bounds_check(index, 0, sizeof...(T));
    return __dispatch_arg{ __dispatch_arg_tuple{ UTL_FORWARD(args)... },
                           index };
}

template <std::size_t N>
auto dispatch_range(std::size_t index) {
    __utl_bounds_check(index, 0, N);
...
template <typename... T>
void __dispatch(__dispatch_arg_tuple<__dispatch_arg<T>...> const& args,
                auto&& selector) {
    __dispatch_impl<sizeof...(T)>(std::integral_constant<std::size_t, 0>{},
                                  UTL_FORWARD(selector), args);
}

template <class T>
void dispatch(
    __dispatch_arg<T> const& a,
...
void dispatch(
    __dispatch_arg<T> const& a, __dispatch_arg<U> const& b,
...
void dispatch(
    __dispatch_arg<T> const& a, __dispatch_arg<U> const& b,
    __dispatch_arg<V> const& c, __dispatch_arg<W> const& d,
    std::invocable<typename __dispatch_arg<T>::__test_type,
                   typename __dispatch_arg<U>::__test_type,
...
void dispatch(
    __dispatch_arg<T> const& a, __dispatch_arg<U> const& b,
    __dispatch_arg<V> const& c, __dispatch_arg<W> const& d,
    __dispatch_arg<X> const& e, __dispatch_arg<Y> const& f,
    std::invocable<typename __dispatch_arg<T>::__test_type,
                   typename __dispatch_arg<U>::__test_type,
                   typename __dispatch_arg<V>::__test_type,
                   typename __dispatch_arg<W>::__test_type,
...
void dispatch(
    ...
    __dispatch(__dispatch_arg_tuple{ a, b, c, d, e, f, g },
               UTL_FORWARD(selector));
}
template <class T, class U, class V, class W, class X, class Y, class Z,
          class A>
void dispatch(
    ...
    __dispatch_arg<V> const& c, __dispatch_arg<W> const& d,
    __dispatch_arg<X> const& e, __dispatch_arg<Y> const& f,
    __dispatch_arg<Z> const& g, __dispatch_arg<A> const& h,
    std::invocable<typename __dispatch_arg<T>::__test_type,
                   typename __dispatch_arg<U>::__test_type,
                   typename __dispatch_arg<V>::__test_type,
                   typename __dispatch_arg<W>::__test_type,
    ...
    __dispatch(__dispatch_arg_tuple{ a, b, c, d, e, f, g, h },
               UTL_FORWARD(selector));
}
