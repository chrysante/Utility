...
class text_style {
  public:
    ...
    FMT_DEPRECATED_NONMSVC FMT_CONSTEXPR text_style       &operator&=(const text_style &rhs) { return and_assign(rhs); }
  ...
  private:
    ...
    friend FMT_CONSTEXPR_DECL text_style fg(detail::color_type foreground) noexcept;

    friend FMT_CONSTEXPR_DECL text_style bg(detail::color_type background) noexcept;
...
template <typename Char> inline void reset_color(FILE *stream) { fputs("\x1b[0m", stream); }

template <> inline void              reset_color<wchar_t>(FILE *stream) { fputs(L"\x1b[0m", stream); }
