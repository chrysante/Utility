...
template <typename OutputIt, typename Char> class printf_arg_formatter: public arg_formatter<Char> {
  private:
  ...
  public:
    ...
    template <typename T, FMT_ENABLE_IF(detail::is_integral<T>::value)> OutputIt operator()(T value) {
    ...
    template <typename T, FMT_ENABLE_IF(std::is_floating_point<T>::value)> OutputIt operator()(T value) {
        return base::operator()(value);
    }
...
template <typename Char> void parse_flags(basic_format_specs<Char> &specs, const Char *&it, const Char *end) {
    for (; it != end; ++it) {
        switch (*it) {
        case '-':
...
inline auto printf(const S &fmt, const T &...args) -> int {
    return vprintf(to_string_view(fmt), fmt::make_format_args<basic_printf_context_t<char_t<S>>>(args...));
}
...
FMT_DEPRECATED auto fprintf(std::basic_ostream<Char> &os, const S &fmt, const T &...args) -> int {
    return vfprintf(os, to_string_view(fmt), fmt::make_format_args<basic_printf_context_t<Char>>(args...));
}
