...
decltype(auto) _decay(auto&& x, bool) {
...
template <typename... Args>
void print(stream_ref_wrapper stream, std::string_view format_string, Args&&... args);

template <typename... Args>
void print(stream_ref_wrapper stream, std::string_view format_string, Args&&... args) {
    fmt::print(*stream._stream, format_string, format_codes::_private::_decay(args, stream._using_colors)...);
}
...
void print(std::string_view format_string, Args&&... args) {
    ...
    std::string formatted = utl::format(format_string, format_codes::_private::_decay(args, stream._using_colors)...);
...
public:
    ...
    constexpr format_code(int value, char const* unicode): _value(value), _unicode(unicode) {}
