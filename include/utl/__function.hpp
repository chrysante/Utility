...
struct nullfunction_t {
    ...
    template <typename Sig>
    operator function<Sig>() const noexcept {
        return {};
    }
    template <typename Sig>
    operator unique_function<Sig>() const noexcept {
        return {};
    }
...
public:
    ...
    // MARK: swap
    void swap(function& other) noexcept { this->storage.swap(other.storage); }
};
...
struct FunctionVTableImpl<F, R(Args...), Deallocator>: FunctionVTable {
    ...
    std::size_t size() const noexcept final { return sizeof(F); }
    std::size_t align() const noexcept final { return alignof(F); }
...
public:
    ...
    // destructor
    ~LocalFunctionObjectStorage() { vTable()->destroy(storagePtr()); }

    void destroy() { this->~LocalFunctionObjectStorage(); }
...
public:
    ...
    void destroy() { this->~HeapFunctionObjectStorage(); }
...
struct NullFunctionStorage {
    void destroy() { this->~NullFunctionStorage(); }
...
public:
    ...
    R invoke(U&&... u) const {
        ...
        if (this->isLocalFunctionObject()) {
            ...
            __utl_assert(!this->isNullFunction(), "this is a nullfunction");
