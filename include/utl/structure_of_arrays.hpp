...
//		}
//
//	The UTL_SOA_TYPE macro then defines multiple structures, especially the
// following:
//
//		struct Particle {
//			int    id;
...
//		};
//
//	It also defines additional template machinery used by the
// utl::structure_of_arrays<Particle> class. 	Subset Types and Subset
// Reference/Pointer Types of the Particle structure are also provided.
//
//	utl::structure_of_arrays<Particle> then behaves similarly to
// std::vector<Particle>, however all 	members of Particle are stored in
// seperate arrays to improve cache locality when iterating over 	individual
// members or subsets of the entire Particle structure. 	Allocations are
// batched, so for the above Particle structure one allocation is performed
// for the for arrays in utl::structure_of_arrays<Particle>. 	All STL
// Algorithms should work with utl::structure_of_arrays<...>.
//
//
...
auto& __soa_get_member(T& t) {
    return t.template get<I>();
}
template <std::size_t I, typename T>
auto const& __soa_get_member(T const& t) {
    return t.template get<I>();
}
template <std::size_t I, typename T>
auto&& __soa_get_member(T&& t) {
...
template <typename T>
struct __soa_type_mapper<__soa_type_kind::value, T> {
    using type = T;
};
template <typename T>
struct __soa_type_mapper<__soa_type_kind::reference, T> {
    using type = T&;
};
template <typename T>
struct __soa_type_mapper<__soa_type_kind::const_reference, T> {
    using type = T const&;
};
template <typename T>
struct __soa_type_mapper<__soa_type_kind::pointer, T> {
    using type = T*;
};
template <typename T>
struct __soa_type_mapper<__soa_type_kind::const_pointer, T> {
    using type = T const*;
};
...
struct __soa_is_full<Meta, std::tuple<T...>>:
    std::bool_constant<
        std::is_same_v<std::make_index_sequence<Meta::member_count>,
                       std::index_sequence<T::index...>> &&
        (!std::is_const_v<T> && ...)> {};

template <typename Meta, typename Members, __soa_type_kind,
...
constexpr __soa_type_kind __make_soa_reference_const_if(bool condition,
                                                        __soa_type_kind k) {
    if (!condition) {
        return k;
    }
...
using __soa_element_member_base =
    typename Meta::template member_base<Member::index>::template type<
        typename __soa_type_mapper<
            __make_soa_reference_const_if(std::is_const_v<Member>, Kind),
            __soa_element_type<Meta, Member::index>>::type>;

template <typename Meta, typename Members, __soa_type_kind Kind>
struct __soa_partial_element;

template <typename Meta, typename... Members, __soa_type_kind Kind>
struct __soa_partial_element<Meta, std::tuple<Members...>, Kind>:
    __soa_element_member_base<Meta, Members, Kind>... {
    ...
    static constexpr bool __this_is_reference =
        Kind == __soa_type_kind::reference ||
        Kind == __soa_type_kind::const_reference;
    static constexpr bool __this_is_pointer =
        Kind == __soa_type_kind::pointer ||
        Kind == __soa_type_kind::const_pointer;

    static constexpr auto __reference_kind = [] {
        if (Kind == __soa_type_kind::reference ||
            Kind == __soa_type_kind::value || Kind == __soa_type_kind::pointer)
        {
    ...
    static constexpr auto __add_ref_helper() {
        ...
        if (Kind == __soa_type_kind::pointer ||
            Kind == __soa_type_kind::const_pointer)
        {
            if (std::is_const_v<M>) {
                return __soa_type_kind::const_pointer;
    ...
    using __add_ref =
        typename __soa_type_mapper<__add_ref_helper<M>(), T>::type;

    constexpr __soa_partial_element(
        __add_ref<Members,
                  __soa_element_type<Meta, Members::index>>... members):
        __member_base<Members>{ members }... {}

    template <__soa_type_kind K>
    using __corresponding_type =
        typename __soa_type_maker<Meta, std::tuple<Members...>, K>::type;

    /// Operator=
    constexpr __soa_partial_element&
    operator=(__corresponding_type<__soa_type_kind::const_reference> const& rhs)
    requires(Kind != __soa_type_kind::const_reference)
    {
        ((this->__member_base<Members>::__get() =
              rhs.__corresponding_type<__soa_type_kind::const_reference>::
                  template __member_base<Members>::__get()),
         ...);
        return *this;
    }
    ...
    constexpr
    operator __corresponding_type<__soa_type_kind::const_reference>() const
    requires(!__this_is_pointer)
    {
        return { __member_base<Members>::__get()... };
    }

    /// Operator&
    constexpr __corresponding_type<__pointer_kind> operator&() const
    requires(__this_is_reference)
    {
        return UTL_WITH_INDEX_SEQUENCE((I, __member_count), {
            return __corresponding_type<__pointer_kind>{ &get<I>()... };
        });
    }
    constexpr __soa_partial_element* operator&()
    requires(__this_is_value)
    {
        return this;
    }
    constexpr __soa_partial_element const* operator&() const
    requires(__this_is_value)
    {
        return this;
    }
    ...
    /// Operator*
    constexpr __corresponding_type<__reference_kind> operator*() const
    requires(__this_is_pointer)
    {
        return UTL_WITH_INDEX_SEQUENCE((I, __member_count), {
            return __corresponding_type<__reference_kind>{ *get<I>()... };
        });
    }

    template <std::size_t I>
    auto& get()
    requires(Kind == __soa_type_kind::value)
    {
...
struct __soa_type_maker<Meta, std::tuple<Member>, __soa_type_kind::value,
                        false> {
    using _T = std::tuple_element_t<Member::index, typename Meta::tuple>;
    using type = _T;
};
template <typename Meta, typename Member>
struct __soa_type_maker<Meta, std::tuple<Member>, __soa_type_kind::reference,
                        false> {
    using _T = std::tuple_element_t<Member::index, typename Meta::tuple>;
    using type = std::conditional_t<std::is_const_v<Member>, _T const&, _T&>;
};
template <typename Meta, typename Member>
struct __soa_type_maker<Meta, std::tuple<Member>,
                        __soa_type_kind::const_reference, false> {
    using _T = std::tuple_element_t<Member::index, typename Meta::tuple>;
    using type = _T const&;
};
template <typename Meta, typename Member>
struct __soa_type_maker<Meta, std::tuple<Member>, __soa_type_kind::pointer,
                        false> {
    using _T = std::tuple_element_t<Member::index, typename Meta::tuple>;
    using type = std::conditional_t<std::is_const_v<Member>, _T const*, _T*>;
};
template <typename Meta, typename Member>
struct __soa_type_maker<Meta, std::tuple<Member>,
                        __soa_type_kind::const_pointer, false> {
    using _T = std::tuple_element_t<Member::index, typename Meta::tuple>;
    using type = _T const*;
};
...
struct std::tuple_element<
    N, utl::__soa_partial_element<Meta, std::tuple<Members...>,
                                  utl::__soa_type_kind::value>> {
...
struct std::tuple_element<
    N, utl::__soa_partial_element<Meta, std::tuple<Members...>,
                                  utl::__soa_type_kind::const_reference>> {
    using __member = std::tuple_element_t<N, std::tuple<Members...>>;
    using _T = std::tuple_element_t<__member::index, typename Meta::tuple>;
    using type = _T const&;
};
...
template <typename Meta, typename MemberIndices, typename T, typename I,
          typename Allocator, __utl_soa_options>
struct __soa_impl;

/// SOA
template <typename ST, typename Allocator,
          typename I =
...
struct __soa_iterator<SOAImpl, std::tuple<T...>, std::index_sequence<I...>,
                      IsConst> {
    using __soa_ref_type =
        std::conditional_t<IsConst, typename SOAImpl::const_reference,
                           typename SOAImpl::reference>;
    using __soa_ptr_type =
        std::tuple<std::conditional_t<IsConst, T const*, T*>...>;

    /// Member Types
    using iterator_category = std::random_access_iterator_tag;
    using difference_type = std::ptrdiff_t;
    using value_type = typename SOAImpl::value_type;
    using pointer = std::conditional_t<IsConst, typename SOAImpl::const_pointer,
                                       typename SOAImpl::pointer>;
    using reference =
        std::conditional_t<IsConst, typename SOAImpl::const_reference,
                           typename SOAImpl::reference>;
    ;
    ...
    operator __soa_iterator<SOAImpl, std::tuple<T...>,
                            std::index_sequence<I...>, true>() const
    requires(!IsConst)
    {
    ...
    operator __soa_iterator<SOAImpl, std::tuple<U...>,
                            std::index_sequence<I...>, RHSIsConst>() const
    requires(!IsConst)
    {
        return { __private_tag{}, std::get<I>(_ptr)... };
    }
    ...
    /// Dereference
    __soa_ref_type operator*() const { return { *std::get<I>(_ptr)... }; }

    /**
     operator-> is not usable. It requires to return a raw pointer, but we can't
     form a raw pointer to our value type, because of the memory layout.
     */
    /*
    ...
    bool operator!=(__soa_iterator const& rhs) const { return !(*this == rhs); }
...
struct alignas(__soa_max_align) __soa_max_aligned_storage:
    std::integral_constant<std::size_t, __soa_max_align> {};

template <typename Meta, typename... _Members, typename... T, std::size_t... I,
          typename _Allocator, __utl_soa_options Options>
struct __soa_impl<Meta, std::tuple<_Members...>, std::tuple<T...>,
                  std::index_sequence<I...>, _Allocator, Options>:
    ...
    using __allocator =
        typename __soa_rebind_allocator<_Allocator,
                                        __soa_max_aligned_storage>::type;
    ...
    static constexpr std::array<std::size_t, __member_count>
    __make_index_map() {
        ...
        for (std::size_t i = 0; i < __member_count; ++i) {
            auto itr =
                std::find(member_indices.begin(), member_indices.end(), i);
    ...
    static constexpr std::array<std::size_t, __member_count> __index_map =
        __make_index_map();
    ...
    static constexpr bool __has_member() {
        ...
        if (!std::is_const_v<Member>) {
            return !std::is_const_v<
                std::tuple_element_t<mapped_index, __members>>;
        }
        return true;
    }

    /// MARK: Member Types
    using value_type = typename __soa_type_maker<Meta, __members,
                                                 __soa_type_kind::value>::type;
    using allocator_type = _Allocator;
    using size_type = std::size_t;
    using difference_type = std::ptrdiff_t;
    using reference =
        typename __soa_type_maker<Meta, __members,
                                  __soa_type_kind::reference>::type;
    using const_reference =
        typename __soa_type_maker<Meta, __members,
                                  __soa_type_kind::const_reference>::type;
    using pointer = typename __soa_type_maker<Meta, __members,
                                              __soa_type_kind::pointer>::type;
    using const_pointer =
        typename __soa_type_maker<Meta, __members,
                                  __soa_type_kind::const_pointer>::type;
    using iterator = __soa_iterator<__soa_impl, std::tuple<T...>,
                                    std::index_sequence<I...>, false>;
    using const_iterator = __soa_iterator<__soa_impl, std::tuple<T const...>,
                                          std::index_sequence<I...>, true>;
    ...
    __soa_impl(std::size_t size, __allocator const& alloc = {}):
        __allocator(alloc) {
        reserve(size);
        this->_size = size;
        (__construct(__begin<I>(), __end<I>()), ...);
    }
    __soa_impl(std::initializer_list<value_type> ilist,
               __allocator const& alloc = {}):
        __allocator(alloc) {
        reserve(ilist.size());
    ...
    /// Copy Constructor
    __soa_impl(__soa_impl const& rhs, __allocator const& alloc = {}):
        __allocator(alloc) {
        if constexpr (__is_container) {
            reserve(rhs._size);
            this->_size = rhs._size;
            (__uninitialized_copy(rhs.__begin<I>(), rhs.__end<I>(),
                                  __begin<I>()),
             ...);
        }
        else {
    ...
    /// Move Constructor
    __soa_impl(__soa_impl&& rhs, __allocator const& alloc = {}) noexcept:
        __allocator(alloc) {
        if constexpr (__is_container) {
            if (__get_allocator() == rhs.__get_allocator()) {
                ...
                reserve(rhs.size());
                this->_size = rhs.size();
                (__uninitialized_move(rhs.__begin<I>(), rhs.__end<I>(),
                                      __begin<I>()),
                 ...);
            }
        }
    ...
    explicit __soa_impl(__private_tag, std::tuple<T*...> ptr, std::size_t size)
    requires(__is_view)
        : __data_base{ ptr, size } {}
    ...
    requires(__is_container)
    {
        __emplace_back_impl(UTL_FORWARD(u)...);
    }
    void emplace_back(T const&... t)
    requires(__is_container)
    {
        __emplace_back_impl(t...);
    }
    void emplace_back(T&&... t)
    requires(__is_container)
    {
        __emplace_back_impl(std::move(t)...);
    }
    ...
    requires(__is_container)
    {
        emplace_back(v.template get<I>()...);
    }
    void push_back(value_type&& v)
    requires(__is_container)
    {
        emplace_back(std::move(v.template get<I>())...);
    }
    ...
    void __insert_impl(std::size_t index, It begin, It end) {
        ...
        // somewhat naive implementation, doing unnecesary copies and shifts
        // when reallocating
    ...
    /// Insert
    void insert(std::size_t index, value_type const& value)
    requires(__is_container)
    {
        __insert_impl(index, &value, &value + 1);
    }

    void insert(std::size_t index, value_type&& value)
    requires(__is_container)
    {
        __insert_impl(index, std::make_move_iterator(&value),
                      std::make_move_iterator(&value + 1));
    }

    template <input_iterator_for<value_type> It>
    void insert(std::size_t index, It begin, It end)
    requires(__is_container)
    {
        __insert_impl(index, begin, end);
    }

    void insert(const_iterator itr, value_type const& value)
    requires(__is_container)
    {
        insert(__itr_to_index(itr), value);
    }

    void insert(const_iterator itr, value_type&& value)
    requires(__is_container)
    {
    ...
    template <input_iterator_for<value_type> It>
    void insert(const_iterator itr, It begin, It end)
    requires(__is_container)
    {
    ...
    void erase(const_iterator first, const_iterator const& last)
    requires(__is_container)
    {
    ...
    /// Swap
    void swap(structure_of_arrays<__soa_type>& rhs)
    requires(__is_container)
    {
        if (__get_allocator() == rhs.__get_allocator()) {
            // we can swap pointers
    ...
    [[nodiscard]] soa_view<__soa_type, Members...> __view_impl() const {
        ...
        return view_t(__private_tag{},
                      { __begin<__index_map[Members::index]>()... }, size());
    }

    /// Owning
    template <typename... Members>
    requires(__has_member<Members>() && ...) && (__is_container)
    [[nodiscard]] auto view() {
        return __view_impl<Members...>();
    }
    template <typename... Members>
    requires(__has_member<Members const>() && ...) && (__is_container)
    [[nodiscard]] auto view() const {
        return __view_impl<Members const...>();
    }
    /// Non-Owning
    template <typename... Members>
    requires(__has_member<Members>() && ...) && (__is_view)
    [[nodiscard]] auto view() const {
        return __view_impl<Members...>();
    }
    ...
    /// Convert to std::span
    operator std::span<std::tuple_element_t<0, std::tuple<T...>>>() const
    requires(sizeof...(T) == 1)
    {
        return std::span<
            std::tuple_element_t<0, std::tuple<T...>>>(__begin<0>(), size());
    }
    operator std::span<std::tuple_element_t<0, std::tuple<T...>> const>() const
    requires(sizeof...(T) == 1) &&
            (!std::is_const_v<std::tuple_element_t<0, std::tuple<T...>>>)
    {
        return std::span<
            std::tuple_element_t<0, std::tuple<T...>>>(__begin<0>(), size());
    }
    ...
    allocator_type get_allocator() const {
        return allocator_type(__get_allocator());
    }
    ...
    auto* __data_impl() const {
        ...
        using V =
            std::conditional_t<std::is_const_v<M> || ForceConst, U const, U>;
        return const_cast<V*>(__begin<tuple_index>());
    }

    template <typename Member>
    requires(__has_member<Member>()) && (!__is_view)
    auto* data() {
    ...
    /// MARK: Begin, End
    std::tuple<T*...> __data_offset(std::ptrdiff_t offset)
    requires(__is_container)
    {
        return { (__begin<I>() + offset)... };
    }
    std::tuple<T const*...> __data_offset(std::ptrdiff_t offset) const
    requires(__is_container)
    {
        return { (__begin<I>() + offset)... };
    }
    std::tuple<T*...> __data_offset(std::ptrdiff_t offset) const
    requires(__is_view)
    {
        return { (__begin<I>() + offset)... };
    }
    ...
    requires(__is_container)
    {
        return iterator(__data_offset(size()));
    }
    const_iterator end() const
    requires(__is_container)
    {
        return const_iterator(__data_offset(size()));
    }
    iterator end() const
    requires(__is_view)
    {
    ...
    requires(__is_container)
    {
        return __begin<M::index, std::is_const_v<M>>();
    }
    template <typename M>
    auto begin() const
    requires(__is_container)
    {
        return __begin<M::index, true>();
    }
    template <typename M>
    auto begin() const
    requires(__is_view)
    ...
    requires(__is_container)
    {
        return __end<M::index, std::is_const_v<M>>();
    }
    template <typename M>
    auto end() const
    requires(__is_container)
    {
        return __end<M::index, true>();
    }
    template <typename M>
    auto end() const
    requires(__is_view)
    ...
    auto* __begin() const {
        ...
        using MaybeConstU =
            std::conditional_t<ForceConst, std::add_const_t<U>, U>;
    ...
    void __construct_at(auto address, auto&&... args) {
        std::allocator_traits<__allocator>::construct(__get_allocator(),
                                                      address,
                                                      UTL_FORWARD(args)...);
    }
    ...
    static __buffer_info __make_buffer_info(std::size_t obj_count) {
        ...
        static constexpr std::array<std::size_t, __member_count + 1>
            alignment = {
                alignof(T)...,
                1 /* append 1 here to make algorithm below simpler, because we
                     don't have to branch on last element */
            };
        static constexpr std::size_t max_align =
            *std::max_element(alignment.begin(), alignment.end());
        ...
        for (std::size_t i = 0; i < __member_count; ++i) {
            ...
            total_size =
                round_up_pow_two(total_size + i_size, alignment[i + 1]);
        }
    ...
    static std::tuple<T*...>
    __decompose_buffer_pointer(void* ptr, __buffer_info const& info) {
    ...
    void* __allocate_raw(std::size_t size, std::size_t alignment) {
        ...
        __utl_assert((std::uintptr_t)result % alignment == 0,
                     "memory not aligned");
    ...
    void __deallocate_raw(void* ptr, std::size_t size, std::size_t alignment) {
        ...
        __get_allocator()
            .deallocate(reinterpret_cast<__soa_max_aligned_storage*>(ptr),
                        size);
    }
    ...
    std::tuple<T*...> __allocate(std::size_t obj_count) {
        ...
        void* const buffer =
            __allocate_raw(buffer_info.total_size, buffer_info.base_align);
    ...
    __allocator& __get_allocator() { return static_cast<__allocator&>(*this); }
...
void swap(structure_of_arrays<ST, Allocator>& a,
          structure_of_arrays<ST, Allocator>& b) {
    a.swap(b);
}
