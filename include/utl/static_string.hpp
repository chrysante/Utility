...
public:
    ...
    requires(N < Cap) constexpr basic_static_string(char const (&str)[N]):
...
requires(N != M) constexpr bool operator==(basic_static_string<N> const& a, basic_static_string<M> const& b) {
