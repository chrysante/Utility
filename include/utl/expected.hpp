...
#define TL_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T)                         \
    TL_NAMESPACE_NAME::detail::is_trivially_copy_constructible<T>
#define TL_EXPECTED_IS_TRIVIALLY_COPY_ASSIGNABLE(T)                            \
...
public:
    ...
    unexpected() = delete;
    constexpr explicit unexpected(E const& e): m_val(e) {}
...
struct expected_operations_base: expected_storage_base<T, E> {
    ...
    template <class U = T,
              detail::enable_if_t<
                  std::is_nothrow_copy_constructible<U>::value>* = nullptr>
    ...
    // unexpected value into temporary storage, then try to copy the T into
    // place. If the construction succeeds, then everything is fine, but if it
    // throws, then we move the old unexpected value back into place before
...
#ifndef TL_EXPECTED_NO_CONSTRR
    constexpr unexpected<E> const&& geterr() const&& {
        return std::move(this->m_unexpect);
    }
#endif
...
struct expected_move_assign_base<T, E, false>: expected_copy_assign_base<T, E> {
    ...
    operator=(expected_move_assign_base const& rhs) = default;
...
struct expected_delete_ctor_base {
    ...
    operator=(expected_delete_ctor_base const&) = default;
    expected_delete_ctor_base&
...
struct expected_delete_assign_base {
    ...
    operator=(expected_delete_assign_base const&) = default;
    expected_delete_assign_base&
...
struct expected_delete_assign_base<T, E, false, true> {
    ...
    expected_delete_assign_base(expected_delete_assign_base&&) noexcept =
        default;
    expected_delete_assign_base&
    operator=(expected_delete_assign_base const&) = delete;
    expected_delete_assign_base&
    operator=(expected_delete_assign_base&&) noexcept = default;
};
...
class expected:
    ...
    unexpected<E> const* errptr() const {
...
public:
    ...
    template <class F>
    constexpr decltype(expected_map_impl(std::declval<expected const&>(),
                                         std::declval<F&&>()))
    map(F&& f) const& {
        return expected_map_impl(*this, std::forward<F>(f));
    }
    ...
    template <class F>
    constexpr decltype(map_error_impl(std::declval<expected const&>(),
                                      std::declval<F&&>()))
    map_error(F&& f) const& {
        return map_error_impl(*this, std::forward<F>(f));
    }
    ...
    template <class G = E,
              detail::enable_if_t<std::is_constructible<E, G const&>::value>* =
                  nullptr,
              detail::enable_if_t<!std::is_convertible<G const&, E>::value>* =
                  nullptr>
    ...
    template <class U, class G,
              detail::enable_if_t<!(std::is_convertible<U const&, T>::value &&
                  ...
                  nullptr,
              detail::expected_enable_from_other<T, E, U, G, U const&,
                                                 G const&>* = nullptr>
    ...
    template <class G = E, detail::enable_if_t<
                               std::is_nothrow_copy_constructible<G>::value &&
...
public:
    ...
    constexpr T const* operator->() const { return valptr(); }
    ...
    constexpr E const& error() const& { return err().value(); }
    TL_EXPECTED_11_CONSTEXPR E& error() & { return err().value(); }
    constexpr E const&& error() const&& { return std::move(err().value()); }
    ...
    constexpr T value_or(U&& v) const& {
        static_assert(std::is_copy_constructible<T>::value &&
                      ...
                      "T must be copy-constructible and convertible to from "
                      "U&&");
    ...
    TL_EXPECTED_11_CONSTEXPR T value_or(U&& v) && {
        static_assert(std::is_move_constructible<T>::value &&
                          std::is_convertible<U&&, T>::value,
                      "T must be move-constructible and convertible to from "
                      "U&&");
