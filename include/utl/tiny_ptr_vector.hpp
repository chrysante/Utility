...
public:
    ...
    /// (1)
    tiny_ptr_vector(): tiny_ptr_vector(allocator_type()) {}

    /// (2)
    explicit tiny_ptr_vector(allocator_type const& alloc):
        ptr(empty_state()), alloc(alloc) {}

    /// (3) & (4)
    explicit tiny_ptr_vector(std::size_t count, T const& value = T(),
    ...
    void __copy_move_ctor_impl(tiny_ptr_vector const& other, auto&& heap_case) {
        switch (other.size()) {
        case 0: break;
    ...
    /// (6)
    tiny_ptr_vector(tiny_ptr_vector const& other):
        tiny_ptr_vector(
    ...
    /// (7)
    tiny_ptr_vector(tiny_ptr_vector const& other, allocator_type const& alloc):
        tiny_ptr_vector(alloc) {
        __copy_move_ctor_impl(other, [&] {
    ...
    /// (8)
    tiny_ptr_vector(tiny_ptr_vector&& other) noexcept:
        tiny_ptr_vector(other.get_allocator()) {
        __copy_move_ctor_impl(other, [&] {
            /// For now
    ...
    /// (9)
    tiny_ptr_vector(tiny_ptr_vector&& other,
    ...
    /// (10)
    tiny_ptr_vector(std::initializer_list<T> init,
                    allocator_type const& alloc = allocator_type()):
        tiny_ptr_vector(alloc) {
        switch (init.size()) {
        case 0: break;
    ...
    /// (1)
    tiny_ptr_vector& operator=(tiny_ptr_vector const& other) {
        if (!other.is_heap()) {
            if (is_heap()) {
                delete_heap(get_heap());
            }
    ...
    /// (2)
    tiny_ptr_vector& operator=(tiny_ptr_vector&& other) noexcept {
        if (!other.is_heap()) {
            if (is_heap()) {
                delete_heap(get_heap());
            }
    ...
    /// (3)
    tiny_ptr_vector& operator=(std::initializer_list<value_type> ilist) {
        assign(ilist);
        return *this;
    }
    ...
    iterator begin() {
        return const_cast<iterator>(
            static_cast<tiny_ptr_vector const*>(this)->begin());
    }
    ...
    iterator end() {
        return const_cast<iterator>(
            static_cast<tiny_ptr_vector const*>(this)->end());
    }
    ...
    reference front() {
        return const_cast<reference>(
            static_cast<tiny_ptr_vector const*>(this)->front());
    }
    ...
    reference back() {
        return const_cast<reference>(
            static_cast<tiny_ptr_vector const*>(this)->back());
    }
    ...
    reference operator[](std::size_t pos) {
        return const_cast<reference>(
            static_cast<tiny_ptr_vector const&>(*this)[pos]);
    }
    ...
    T* data() noexcept {
        return const_cast<T*>(
            static_cast<tiny_ptr_vector const*>(this)->data());
    }
    ...
    void swap(tiny_ptr_vector& other) {
        __utl_assert(alloc == other.alloc);
        void* this_state;
...
private:
    ...
    impl_type* get_heap() {
        return const_cast<impl_type*>(
            static_cast<tiny_ptr_vector const*>(this)->get_heap());
    }
...
bool operator==(tiny_ptr_vector<T, Allocator> const& lhs,
                tiny_ptr_vector<T, Allocator> const& rhs) {
