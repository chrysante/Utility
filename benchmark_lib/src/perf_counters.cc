...
PerfCounters     PerfCounters::Create(const std::vector<std::string> &counter_names) {
        if (counter_names.empty()) {
            return NoCounters();
    }
        if (counter_names.size() > PerfCounterValues::kMaxCounters) {
            GetErrorLogInstance() << counter_names.size() << " counters were requested. The minimum is 1, the maximum is "
        ...
        for (size_t i = 0; i < counter_names.size(); ++i) {
            ...
            const int pfm_get = pfm_get_os_event_encoding(name.c_str(), mode, PFM_OS_PERF_EVENT, &arg);
            ...
            if (id < 0) {
                GetErrorLogInstance() << "Failed to get a file descriptor for " << name << "\n";
                return NoCounters();
        }
...
PerfCountersMeasurement::PerfCountersMeasurement(const std::vector<std::string> &counter_names):
