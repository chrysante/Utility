...
// returns true; otherwise leaves *value unchanged and returns false.
bool ParseKvPairs(const std::string &src_text, const char *str, std::map<std::string, std::string> *value) {
    ...
    for (const auto &kvpair : StrSplit(str, ',')) {
        ...
        if (!kvs.emplace(kv[0], kv[1]).second) {
            std::cerr << src_text << " is expected to contain unique keys but key \"" << kv[0] << "\" was repeated.\n";
            return false;
        }
    }
...
BENCHMARK_EXPORT
bool IsFlag(const char *str, const char *flag) { return (ParseFlagValue(str, flag, true) != nullptr); }
