...
static std::string FormatTime(double time) {
    // For the time columns of the console printer 13 digits are reserved. One
    // of them is a space and max two of them are the time unit (e.g ns). That
    // puts us at 10 digits usable for the number. Align decimal places...
    if (time < 1.0) {
...
void ConsoleReporter::PrintRunData(const Run &result) {
    ...
    PrinterFn *printer    = (output_options_ & OO_Color) ? static_cast<PrinterFn *>(ColorPrintf) : IgnoreColorPrint;
    ...
    } else if (result.run_type != Run::RT_Aggregate || result.aggregate_unit == StatisticUnit::kTime) {
    ...
    for (auto &c : result.counters) {
        ...
        std::string       s;
        const char       *unit = "";
        if (result.run_type == Run::RT_Aggregate && result.aggregate_unit == StatisticUnit::kPercentage) {
