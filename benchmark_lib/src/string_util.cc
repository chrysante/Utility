...
std::string HumanReadableNumber(double n, double one_k) {
    // 1.1 means that figures up to 1.1k should be shown with the next unit
    // down; this softens edge effects. 1 means that we should show one decimal
    // place of precision.
...
std::string StrFormatImp(const char *msg, va_list args) {
    ...
    // 2015-10-08: vsnprintf is used instead of snd::vsnprintf due to a
    // limitation in the android-ndk
...
std::vector<std::string> StrSplit(const std::string &str, char delim) {
    ...
    for (; next != std::string::npos; first = next + 1, next = str.find(delim, first)) {
