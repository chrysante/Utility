...
#define BM_DECLARE_kvpairs(name) BENCHMARK_EXPORT extern std::map<std::string, std::string> FLAG(name)

// Macros for defining flags.
#define BM_DEFINE_bool(name, default_val) BENCHMARK_EXPORT bool FLAG(name) = benchmark::BoolFromEnv(#name, default_val)
#define BM_DEFINE_int32(name, default_val)                                                                             \
    BENCHMARK_EXPORT int32_t FLAG(name) = benchmark::Int32FromEnv(#name, default_val)
#define BM_DEFINE_double(name, default_val)                                                                            \
    BENCHMARK_EXPORT double FLAG(name) = benchmark::DoubleFromEnv(#name, default_val)
#define BM_DEFINE_string(name, default_val)                                                                            \
    BENCHMARK_EXPORT std::string FLAG(name) = benchmark::StringFromEnv(#name, default_val)
#define BM_DEFINE_kvpairs(name, default_val)                                                                           \
    BENCHMARK_EXPORT std::map<std::string, std::string> FLAG(name) = benchmark::KvPairsFromEnv(#name, default_val)
