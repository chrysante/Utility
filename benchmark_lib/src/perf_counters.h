...
class PerfCounterValues {
  ...
  private:
    ...
    std::pair<char *, size_t> get_data_buffer() {
        return {reinterpret_cast<char *>(values_.data()), sizeof(uint64_t) * (kPadding + nr_counters_)};
    }
...
class BENCHMARK_EXPORT PerfCountersMeasurement final {
  public:
    ...
    // decided (either invalid or valid) and won't change again even if a ctor
    // is concurrently running with this. This is preferring efficiency to
    // maintainability, because the address of the static can be known at
    // compile time.
    bool IsValid() const {
        MutexLock l(mutex_);
    ...
    BENCHMARK_ALWAYS_INLINE bool Stop(std::vector<std::pair<std::string, double>> &measurements) {
        assert(IsValid());
        MutexLock l(mutex_);
        // Tell the compiler to not move instructions above/below where we take
        // the snapshot.
        ...
        for (size_t i = 0; i < counters_.names().size(); ++i) {
            double measurement = static_cast<double>(end_values_[i]) - static_cast<double>(start_values_[i]);
