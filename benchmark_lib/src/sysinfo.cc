...
struct ValueUnion {
  ...
  public:
    ...
    explicit ValueUnion(std::size_t buff_size):
        size(sizeof(DataT) + buff_size), buff(::new (std::malloc(size)) DataT(), &std::free) {}
...
CPUInfo::Scaling CpuScaling(int num_cpus) {
    ...
    // local file system. If reading the exported files fails, then we may not
    // be running on Linux, so we silently ignore all the read errors.
    std::string res;
...
std::string GetSystemName() {
    ...
    int len = ::MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, hostname, DWCOUNT, converted.begin(), 0);
    // TODO: Report error from GetLastError()?
    if (len == 0)
        return std::string("");
    converted.reserve(len + 1);

    len = ::MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, hostname, DWCOUNT, converted.begin(), converted.size());
    // TODO: Report error from GetLastError()?
    if (len == 0)
        return std::string("");
...
double GetCPUCyclesPerSecond(CPUInfo::Scaling scaling) {
    ...
    while (std::getline(f, ln)) {
        ...
        // accept positive values. Some environments (virtual machines) report
        // zero, which would cause infinite looping in WallTime_Init.
    ...
    fprintf(stderr, "This does not affect benchmark measurements, only the "
                    "metadata output.\n");
...
CPUInfo::CPUInfo():
    num_cpus(GetNumCPUs()), scaling(CpuScaling(num_cpus)), cycles_per_second(GetCPUCyclesPerSecond(scaling)),
    caches(GetCacheSizes()), load_avg(GetLoadAvg()) {}
