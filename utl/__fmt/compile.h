...
template <typename T, typename Enable = void> struct field_type { using type = remove_cvref_t<T>; };
...
constexpr auto parse_replacement_field_then_tail(S format_str) {
    ...
    if constexpr (c == '}') {
        return parse_tail<Args, END_POS + 1, NEXT_ID>(field<char_type, typename field_type<T>::type, ARG_INDEX>(),
                                                      format_str);
    } else if constexpr (c == ':') {
...
FMT_INLINE std::basic_string<typename S::char_type> format(const S &, Args &&...args) {
    ...
    if constexpr (std::is_same<remove_cvref_t<decltype(compiled)>, detail::unknown_format>()) {
        return format(static_cast<basic_string_view<typename S::char_type>>(S()), std::forward<Args>(args)...);
    } else {
...
FMT_CONSTEXPR OutputIt format_to(OutputIt out, const S &, Args &&...args) {
    constexpr auto compiled = detail::compile<Args...>(S());
    if constexpr (std::is_same<remove_cvref_t<decltype(compiled)>, detail::unknown_format>()) {
        return format_to(out, static_cast<basic_string_view<typename S::char_type>>(S()), std::forward<Args>(args)...);
    } else {
...
format_to_n_result<OutputIt> format_to_n(OutputIt out, size_t n, const S &format_str, Args &&...args) {
    auto it = format_to(detail::truncating_iterator<OutputIt>(out, n), format_str, std::forward<Args>(args)...);
