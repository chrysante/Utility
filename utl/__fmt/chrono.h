...
template <typename OutputIt, typename Char> class tm_writer {
  ...
  public:
    ...
    void on_iso_date() {
        ...
        write_digit2_separated(buf + 2, static_cast<unsigned>(year % 100), to_unsigned(tm_mon() + 1),
                               to_unsigned(tm_mday()), '-');
...
template <long long Num, long long Den, int N = 0,
          bool Enabled = (N < 19) && (Num <= std::numeric_limits<long long>::max() / 10)>
        ...
        if (ns == numeric_system::standard) {
            if (std::is_floating_point<rep>::value) {
                ...
                format_to(std::back_inserter(buf), runtime("{:.{}f}"),
                          std::fmod(val * static_cast<rep>(Period::num) / static_cast<rep>(Period::den), 60),
...
struct formatter<std::chrono::time_point<std::chrono::system_clock, Duration>, Char>: formatter<std::tm, Char> {
    FMT_CONSTEXPR formatter() { this->do_parse(default_specs, default_specs + sizeof(default_specs) / sizeof(Char)); }
...
template <typename Char, typename Duration>
constexpr const Char formatter<std::chrono::time_point<std::chrono::system_clock, Duration>, Char>::default_specs[];
...
template <typename Char> struct formatter<std::tm, Char> {
  ...
  protected:
    template <typename It> FMT_CONSTEXPR auto do_parse(It begin, It end, bool with_default = false) -> It {
        if (begin != end && *begin == ':')
            ++begin;
