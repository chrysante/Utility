...
#pragma GCC diagnostic pop
    ...
    static constexpr uint64_t power_of_10_64[20] = {1, FMT_POWERS_OF_10(1ULL), FMT_POWERS_OF_10(1000000000ULL),
...
template <typename T> constexpr uint64_t basic_impl_data<T>::pow10_significands[];
...
struct fp {
    ...
    // successor.
    template <typename Float, FMT_ENABLE_IF(is_supported<Float>::value)> FMT_CONSTEXPR bool assign(Float n) {
    ...
    template <typename Float, FMT_ENABLE_IF(!is_supported<Float>::value)> bool assign(Float) {
        FMT_ASSERT(false, "");
        return false;
    }
};
...
struct gen_digits_handler {
    ...
    FMT_CONSTEXPR digits::result on_digit(char digit, uint64_t divisor, uint64_t remainder, uint64_t error,
                                          bool integral) {
...
FMT_INLINE FMT_CONSTEXPR20 digits::result grisu_gen_digits(fp value, uint64_t error, int &exp,
    ...
    do {
        ...
        auto result    = handler.on_digit(static_cast<char>('0' + digit), impl_data::power_of_10_64[exp] << -one.e,
...
struct uint128_wrapper {
    ...
    constexpr uint128_wrapper(uint64_t high, uint64_t low) noexcept: high_{high}, low_{low} {}
...
inline uint128_wrapper umul192_upper128(uint64_t x, uint128_wrapper y) noexcept {
...
inline uint64_t umul96_upper64(uint32_t x, uint64_t y) noexcept { return umul128_upper64(uint64_t(x) << 32, y); }

// Computes lower 128 bits of multiplication of a 64-bit unsigned integer and a
// 128-bit unsigned integer.
inline uint128_wrapper umul192_lower128(uint64_t x, uint128_wrapper y) noexcept {
...
inline uint32_t divide_by_10_to_kappa_plus_1(uint32_t n) noexcept { return n / float_info<float>::big_divisor; }
...
template <> struct cache_accessor<float> {
    ...
    static uint32_t compute_delta(const cache_entry_type &cache, int beta_minus_1) noexcept {
    ...
    static compute_mul_parity_result compute_mul_parity(carrier_uint two_f, const cache_entry_type &cache,
        ...
        return {((r >> (64 - beta_minus_1)) & 1) != 0, static_cast<uint32_t>(r >> (32 - beta_minus_1)) == 0};
    }

    static carrier_uint compute_left_endpoint_for_shorter_interval_case(const cache_entry_type &cache,
                                                                        int                     beta_minus_1) noexcept {
        return static_cast<carrier_uint>((cache - (cache >> (float_info<float>::significand_bits + 2))) >>
                                         (64 - float_info<float>::significand_bits - 1 - beta_minus_1));
    }

    static carrier_uint compute_right_endpoint_for_shorter_interval_case(const cache_entry_type &cache,
                                                                         int beta_minus_1) noexcept {
        return static_cast<carrier_uint>((cache + (cache >> (float_info<float>::significand_bits + 1))) >>
                                         (64 - float_info<float>::significand_bits - 1 - beta_minus_1));
    }

    static carrier_uint compute_round_up_for_shorter_interval_case(const cache_entry_type &cache,
                                                                   int                     beta_minus_1) noexcept {
        return (static_cast<carrier_uint>(cache >> (64 - float_info<float>::significand_bits - 2 - beta_minus_1)) + 1) /
               2;
    }
};
...
template <> struct cache_accessor<double> {
    ...
    static uint32_t compute_delta(cache_entry_type const &cache, int beta_minus_1) noexcept {
...
template <class T> bool is_left_endpoint_integer_shorter_interval(int exponent) noexcept {
...
template <class T> FMT_INLINE decimal_fp<T> shorter_interval_case(int exponent) noexcept {
...
template <typename T> decimal_fp<T> to_decimal(T x) noexcept {
    ...
    if (r > deltai) {
        ...
        if (!include_left_endpoint || exponent < float_info<T>::case_fc_pm_half_lower_threshold ||
            ...
            // x is not an integer, so if z^(f) >= delta^(f) (even parity), we
            // in fact have strict inequality.
            if (!cache_accessor<T>::compute_mul_parity(two_fl, cache, beta_minus_1).parity) {
                goto small_divisor_case_label;
            }
        } else {
...
FMT_CONSTEXPR20 inline void format_dragon(fp value, bool is_predecessor_closer, int num_digits, buffer<char> &buf,
                                          int &exp10) {
...
template <typename Float>
FMT_HEADER_ONLY_CONSTEXPR20 int format_float(Float value, int precision, float_specs specs, buffer<char> &buf) {
    ...
    if (use_dragon) {
        auto f                     = fp();
        bool is_predecessor_closer = specs.binary32 ? f.assign(static_cast<float>(value)) : f.assign(value);
...
template <> struct formatter<detail::bigint> {
    FMT_CONSTEXPR format_parse_context::iterator parse(format_parse_context &ctx) { return ctx.begin(); }
