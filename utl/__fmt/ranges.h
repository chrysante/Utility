...
template <typename OutputIterator> OutputIterator copy(wchar_t ch, OutputIterator out) {
    *out++ = ch;
    return out;
}
...
template <typename T> class is_set {
  ...
  public:
    ...
    static FMT_CONSTEXPR_DECL const bool value = false;
#else
    static FMT_CONSTEXPR_DECL const bool value = !std::is_void<decltype(check<T>(nullptr))>::value && !is_map<T>::value;
#endif
};
...
template <typename T>
struct has_const_begin_end<T, void_t<decltype(detail::range_begin(std::declval<const remove_cvref_t<T> &>())),
                                     decltype(detail::range_end(std::declval<const remove_cvref_t<T> &>()))>>
    : std::true_type {};

template <typename T>
struct has_mutable_begin_end<
    T, void_t<decltype(detail::range_begin(std::declval<T>())), decltype(detail::range_end(std::declval<T>())),
...
template <class Tuple, class F, size_t... Is> void for_each(index_sequence<Is...>, Tuple &&tup, F &&f) noexcept {
    using std::get;
    ...
    do {
        ...
        default:
            ...
            for (Char escape_char : basic_string_view<Char>(escape.begin, to_unsigned(escape.end - escape.begin))) {
                out = format_to(out, "\\x{:02x}", static_cast<typename std::make_unsigned<Char>::type>(escape_char));
            }
            continue;
        }
        *out++ = c;
...
template <typename T, typename Char> struct is_range {
    static FMT_CONSTEXPR_DECL const bool value = detail::is_range_<T>::value && !detail::is_std_string_like<T>::value &&
...
using range_formatter_type = conditional_t<
    ...
    formatter<remove_cvref_t<decltype(range_mapper<buffer_context<Char>, Element>{}.map(std::declval<Element>()))>,
              Char>,
...
// Workaround a bug in MSVC 2019 and earlier.
                             ...
                             && (is_formattable<detail::uncvref_type<R>, Char>::value ||
                                 detail::has_fallback_formatter<detail::uncvref_type<R>, Char>::value)
#endif
                             >> {

    using formatter_type = detail::range_formatter_type<Char, detail::uncvref_type<R>>;
    ...
    template <typename FormatContext, typename U,
              FMT_ENABLE_IF(std::is_same<U, conditional_t<detail::has_const_begin_end<R>::value, const R, R>>::value)>
...
// Workaround a bug in MSVC 2019 and earlier.
                             ...
                             && (is_formattable<detail::uncvref_type<T>, Char>::value ||
                                 detail::has_fallback_formatter<detail::uncvref_type<T>, Char>::value)
#endif
                             >> {
    template <typename ParseContext> FMT_CONSTEXPR auto parse(ParseContext &ctx) -> decltype(ctx.begin()) {
        return ctx.begin();
    ...
    template <typename FormatContext, typename U,
              FMT_ENABLE_IF(std::is_same<U, conditional_t<detail::has_const_begin_end<T>::value, const T, T>>::value)>
...
template <typename Char, typename... T> using tuple_arg_join = tuple_join_view<Char, T...>;
