...
public:
    dispatch_item(std::invocable auto&& function): _function(UTL_FORWARD(function)) {}
...
public:
    ...
    void add(F&& function) { m_tasks.emplace_back(std::forward<F>(function)); }

    std::size_t size() const { return m_tasks.size(); }

    [[nodiscard]] dispatch_handle get_handle() {
        __utl_expect(!m_promise.has_value(), "get_handle() must not be called more than once");
        m_promise = std::promise<bool>{};
        return dispatch_handle(m_promise->get_future());
    }

    void on_completion(utl::function<void()> f) { m_on_completion = std::move(f); }
    void on_failure(utl::function<void()> f) { m_on_failure = std::move(f); }
...
public:
    ...
    explicit concurrent_dispatch_queue(std::size_t thread_count): thread_pool(thread_count) {}

    void async(dispatch_item&& item) { this->thread_pool::submit(std::move(item)); }

    void async(dispatch_group&&);

    using thread_pool::cancel_current_tasks;
    void wait_for_current_tasks() { this->thread_pool::wait_for_current_tasks(); }
