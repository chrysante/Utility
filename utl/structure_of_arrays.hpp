...
namespace utl {
	...
	private:
		...
		public:
			...
			template <typename U>
			requires(contains<U>::value)
			U const& get() const  { return *std::get<index_of<U>>(m_values); }
			
			operator reference_t<!is_const>() const requires(!is_const) {
				return utl::unsafe_bit_cast<reference_t<true>>(*this);
			}
	...
	public:
		...
		void set_allocator(pmr::polymorphic_allocator<U> alloc) {
			__utl_ensure(this->get_allocator<U>() == alloc || this->capacity() == 0, "Allocator must compare equal to previously set one or container must be deallocated");
