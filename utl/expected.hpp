...
#define TL_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T) TL_NAMESPACE_NAME::detail::is_trivially_copy_constructible<T>
...
struct expected_copy_base<T, E, false>: expected_operations_base<T, E> {
    ...
    expected_copy_base(expected_copy_base const& rhs): expected_operations_base<T, E>(no_init) {
        if (rhs.has_value()) {
...
public:
    ...
    template <class U,
              ...
              detail::enable_if_t<(std::is_convertible<U const&, T>::value &&
                                   std::is_convertible<G const&, E>::value)>*     = nullptr,
              detail::expected_enable_from_other<T, E, U, G, const U&, const G&>* = nullptr>
    TL_EXPECTED_11_CONSTEXPR expected(expected<U, G> const& rhs): ctor_base(detail::default_constructor_tag{}) {
        if (rhs.has_value()) {
            this->construct(*rhs);
        }
        else {
...
private:
    ...
    void swap_where_only_one_has_value(expected& rhs,
                                       t_is_void) noexcept(std::is_nothrow_move_constructible<E>::value) {
...
template <class T, class E, class U>
constexpr bool operator==(expected<T, E> const& x, const U& v) {
    return x.has_value() ? *x == v : false;
}
template <class T, class E, class U>
constexpr bool operator==(const U& v, expected<T, E> const& x) {
    return x.has_value() ? *x == v : false;
}
template <class T, class E, class U>
constexpr bool operator!=(expected<T, E> const& x, const U& v) {
