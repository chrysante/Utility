...
namespace utl {
	...
	class local_pimpl {
	public:
		local_pimpl() requires (std::is_default_constructible_v<Impl>) {
			_construct();
		}
		...
		template <std::derived_from<Impl> Derived, typename... Args> requires (std::is_constructible_v<Derived, Args...>)
		local_pimpl(derived_tag_t<Derived>, Args&&... args)  {
			_construct<Derived>(UTL_FORWARD(args)...);
		}
		
		local_pimpl(local_pimpl const& rhs) requires(std::is_copy_constructible_v<Impl>) {
			_construct(*rhs);
		}
