...
BENCHMARK_EXPORT void Initialize(int *argc, char **argv, void (*HelperPrinterf)() = PrintDefaultHelp);
...
BENCHMARK_EXPORT size_t RunSpecifiedBenchmarks(BenchmarkReporter *display_reporter, std::string spec);

BENCHMARK_EXPORT size_t RunSpecifiedBenchmarks(BenchmarkReporter *display_reporter, BenchmarkReporter *file_reporter);
BENCHMARK_EXPORT size_t RunSpecifiedBenchmarks(BenchmarkReporter *display_reporter, BenchmarkReporter *file_reporter,
                                               std::string spec);
...
class MemoryManager {
  public:
    ...
    struct Result {
        Result():
            num_allocs(0), max_bytes_used(0), total_allocated_bytes(TombstoneValue), net_heap_growth(TombstoneValue) {}
...
#ifdef BENCHMARK_HAS_CXX11
inline BENCHMARK_ALWAYS_INLINE void ClobberMemory() { std::atomic_signal_fence(std::memory_order_acq_rel); }
#endif
...
inline BENCHMARK_ALWAYS_INLINE
    typename std::enable_if<std::is_trivially_copyable<Tp>::value && (sizeof(Tp) <= sizeof(Tp *))>::type
...
#ifndef BENCHMARK_HAS_CXX11
inline BENCHMARK_ALWAYS_INLINE void ClobberMemory() { asm volatile("" : : : "memory"); }
#endif
#elif defined(_MSC_VER)
template <class Tp> inline BENCHMARK_ALWAYS_INLINE void DoNotOptimize(Tp const &value) {
...
class Counter {
  public:
    enum Flags {
        ...
        // iteration. When reporting, it will be *multiplied* by the iteration
        // count.
...
class BENCHMARK_EXPORT State {
  public:
    ...
    // C++11 ranged-based for loop. These functions should not be called
    // directly.
    //
    // REQUIRES: The benchmark has not started running yet. Neither begin nor
    // end have been called previously.
    //
    // NOTE: KeepRunning may not be used after calling either of these
    // functions.
    ...
    // REQUIRES: timer is not running and 'SkipWithError(...)' has not been
    // called
    //           by the current thread.
    ...
    //
    // If the ranged-for style of benchmark loop is used, the user must
    // explicitly break from the loop, otherwise all future iterations will be
    // run. If the 'KeepRunning()' loop is used the current thread will
    // automatically exit the loop at the end of the current iteration.
    //
    // For threaded benchmarks only the current thread stops executing and
    // future calls to `KeepRunning()` will block until all threads have
    // completed the `KeepRunning()` loop. If multiple threads report an error
    // only the first error message is used.
    //
    // NOTE: Calling 'SkipWithError(...)' does not cause the benchmark to exit
    // the current scope immediately. If the function is called from within
    // the 'KeepRunning()' loop the current iteration will finish. It is the
    // users responsibility to exit the scope as needed.
    ...
    void SetBytesProcessed(int64_t bytes) {
        counters["bytes_per_second"] = Counter(static_cast<double>(bytes), Counter::kIsRate, Counter::kIs1024);
    }
    ...
    void SetItemsProcessed(int64_t items) {
        counters["items_per_second"] = Counter(static_cast<double>(items), benchmark::Counter::kIsRate);
    }
    ...
    void BENCHMARK_ALWAYS_INLINE SetLabel(const std::string &str) { this->SetLabel(str.c_str()); }
  ...
  private:
    State(IterationCount max_iters, const std::vector<int64_t> &ranges, int thread_i, int n_threads,
...
inline BENCHMARK_ALWAYS_INLINE bool State::KeepRunning() { return KeepRunningInternal(1, /*is_batch=*/false); }
...
inline BENCHMARK_ALWAYS_INLINE State::StateIterator State::begin() { return StateIterator(this); }
...
class BENCHMARK_EXPORT Benchmark {
  public:
    ...
    // each run with k threads. Time allowing (e.g. for a short benchmark),
    // there may be multiple such runs per benchmark, each run with its own
    // "setup"/"teardown".
    //
    ...
    // Set the minimum amount of time to run the benchmark before taking
    // runtimes of this benchmark into account. This option overrides the
    // `benchmark_min_warmup_time` flag. REQUIRES: `t >= 0` and `Iterations` has
    // not been called on this benchmark.
    ...
    // Specify the amount of times to repeat this benchmark. This option
    // overrides the `benchmark_repetitions` flag. REQUIRES: `n > 0`
    ...
    // time (be it either the CPU time of the main thread only (default), or the
    // total CPU usage of the benchmark), call this method. If called, the
    // elapsed (wall) time will be used to control how many iterations are run,
    // and in the printing of items/second or MB/seconds values. If not called,
    // the CPU time used by the benchmark will be used.
    ...
    // to control how many iterations are run, and in the printing of
    // items/second or MB/second values.
...
#if defined(BENCHMARK_HAS_CXX11)
template <class Lambda> internal::Benchmark *RegisterBenchmark(const char *name, Lambda &&fn);
#endif
...
template <class Lambda> class LambdaBenchmark: public Benchmark {
  ...
  private:
    ...
    LambdaBenchmark(const char *name, OLambda &&lam): Benchmark(name), lambda_(std::forward<OLambda>(lam)) {}
...
inline internal::Benchmark *RegisterBenchmark(const char *name, internal::Function *fn) {
    return internal::RegisterBenchmarkInternal(::new internal::FunctionBenchmark(name, fn));
}

#ifdef BENCHMARK_HAS_CXX11
template <class Lambda> internal::Benchmark *RegisterBenchmark(const char *name, Lambda &&fn) {
    using BenchType = internal::LambdaBenchmark<typename std::decay<Lambda>::type>;
    return internal::RegisterBenchmarkInternal(::new BenchType(name, std::forward<Lambda>(fn)));
}
#endif

#if defined(BENCHMARK_HAS_CXX11) && (!defined(BENCHMARK_GCC_VERSION) || BENCHMARK_GCC_VERSION >= 409)
template <class Lambda, class... Args>
internal::Benchmark *RegisterBenchmark(const char *name, Lambda &&fn, Args &&...args) {
    return benchmark::RegisterBenchmark(name, [=](benchmark::State &st) { fn(st, args...); });
}
#else
...
#ifdef BENCHMARK_HAS_CXX11
#define BENCHMARK_PRIVATE_NAME(...) BENCHMARK_PRIVATE_CONCAT(benchmark_uniq_, BENCHMARK_PRIVATE_UNIQUE_ID, __VA_ARGS__)
#else
...
#ifdef BENCHMARK_HAS_CXX11
#define BENCHMARK(...)                                                                                                 \
    BENCHMARK_PRIVATE_DECLARE(_benchmark_) = (::benchmark::internal::RegisterBenchmarkInternal(                        \
...
#define BENCHMARK_CAPTURE(func, test_case_name, ...)                                                                   \
    BENCHMARK_PRIVATE_DECLARE(func) =                                                                                  \
        (::benchmark::internal::RegisterBenchmarkInternal(new ::benchmark::internal::FunctionBenchmark(                \
            #func "/" #test_case_name, [](::benchmark::State &st) { func(st, __VA_ARGS__); })))
...
#define BENCHMARK_TEMPLATE(n, ...)                                                                                     \
    BENCHMARK_PRIVATE_DECLARE(n) = (::benchmark::internal::RegisterBenchmarkInternal(                                  \
        new ::benchmark::internal::FunctionBenchmark(#n "<" #__VA_ARGS__ ">", n<__VA_ARGS__>)))
#else
...
#define BENCHMARK_PRIVATE_DECLARE_F(BaseClass, Method)                                                                 \
    class BaseClass##_##Method##_Benchmark: public BaseClass {                                                         \
      public:                                                                                                          \
        BaseClass##_##Method##_Benchmark() { this->SetName(#BaseClass "/" #Method); }                                  \
                                                                                                                       \
      protected:                                                                                                       \
...
#define BENCHMARK_TEMPLATE1_PRIVATE_DECLARE_F(BaseClass, Method, a)                                                    \
    class BaseClass##_##Method##_Benchmark: public BaseClass<a> {                                                      \
      public:                                                                                                          \
        BaseClass##_##Method##_Benchmark() { this->SetName(#BaseClass "<" #a ">/" #Method); }                          \
                                                                                                                       \
      protected:                                                                                                       \
        virtual void BenchmarkCase(::benchmark::State &) BENCHMARK_OVERRIDE;                                           \
    };
...
#define BENCHMARK_TEMPLATE2_PRIVATE_DECLARE_F(BaseClass, Method, a, b)                                                 \
    class BaseClass##_##Method##_Benchmark: public BaseClass<a, b> {                                                   \
      public:                                                                                                          \
        BaseClass##_##Method##_Benchmark() { this->SetName(#BaseClass "<" #a "," #b ">/" #Method); }                   \
                                                                                                                       \
      protected:                                                                                                       \
        virtual void BenchmarkCase(::benchmark::State &) BENCHMARK_OVERRIDE;                                           \
    };
...
#define BENCHMARK_TEMPLATE_PRIVATE_DECLARE_F(BaseClass, Method, ...)                                                   \
    class BaseClass##_##Method##_Benchmark: public BaseClass<__VA_ARGS__> {                                            \
      public:                                                                                                          \
        BaseClass##_##Method##_Benchmark() { this->SetName(#BaseClass "<" #__VA_ARGS__ ">/" #Method); }                \
                                                                                                                       \
      protected:                                                                                                       \
        virtual void BenchmarkCase(::benchmark::State &) BENCHMARK_OVERRIDE;                                           \
    };
...
#define BENCHMARK_PRIVATE_REGISTER_F(TestName)                                                                         \
    BENCHMARK_PRIVATE_DECLARE(TestName) = (::benchmark::internal::RegisterBenchmarkInternal(new TestName()))
...
class BENCHMARK_EXPORT BenchmarkReporter {
  public:
    ...
    struct BENCHMARK_EXPORT Run {
        ...
        Run():
            run_type(RT_Iteration), aggregate_unit(kTime), error_occurred(false), iterations(1), threads(1),
            time_unit(GetDefaultTimeUnit()), real_accumulated_time(0), cpu_accumulated_time(0), max_heapbytes_used(0),
            complexity(oNone), complexity_lambda(), complexity_n(0), report_big_o(false), report_rms(false),
            memory_result(NULL), allocs_per_iter(0.0) {}
...
class BENCHMARK_EXPORT BENCHMARK_DEPRECATED_MSG("The CSV Reporter will be removed in a future release") CSVReporter
