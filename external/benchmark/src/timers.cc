...
#include <unistd.h>
#if defined BENCHMARK_OS_FREEBSD || defined BENCHMARK_OS_DRAGONFLY || defined BENCHMARK_OS_MACOSX
...
double MakeTime(struct timespec const &ts) { return ts.tv_sec + (static_cast<double>(ts.tv_nsec) * 1e-9); }
#endif
...
std::string LocalDateTimeString() {
    ...
    // * snprintf with %c%02li:%02li - The maximum length is 41 (one for %c, up
    // to 19 for %02li,
    ...
    if (tz_len < kTzOffsetLen && tz_len > 1) {
        // Timezone offset was written. strftime writes offset as +HHMM or
        // -HHMM, RFC3339 specifies an offset as +HH:MM or -HH:MM. To convert,
        // we parse the offset as an integer, then reprint it to a string.
        ...
        tz_len = ::snprintf(tz_offset, sizeof(tz_offset), "%c%02li:%02li", tz_offset_sign, offset_minutes / 100,
                            offset_minutes % 100);
        BM_CHECK(tz_len == kTzOffsetLen);
        ((void)tz_len); // Prevent unused variable warning in optimized build.
    } else {
        // Unknown offset. RFC3339 specifies that unknown local offsets should
        // be written as UTC time with -00:00 timezone.
#if defined(BENCHMARK_OS_WINDOWS)
