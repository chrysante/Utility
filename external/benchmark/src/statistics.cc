...
std::vector<BenchmarkReporter::Run> ComputeStats(const std::vector<BenchmarkReporter::Run> &reports) {
    ...
    for (const auto &Stat : *reports[0].statistics) {
        ...
        if (data.aggregate_unit == StatisticUnit::kTime) {
            // We will divide these times by data.iterations when reporting, but
            // the data.iterations is not necessarily the scale of these
            // measurements, because in each repetition, these timers are sum
            // over all the iters. And if we want to say that the stats are over
            // N repetitions and not M iterations, we need to multiply these by
            // (N/M).
        ...
        for (auto const &kv : counter_stats) {
            // Do *NOT* rescale the custom counters. They are already properly
            // scaled.
