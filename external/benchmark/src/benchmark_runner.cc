...
void RunInThread(const BenchmarkInstance *b, IterationCount iters, int thread_id, ThreadManager *manager,
    ...
    State                 st = b->Run(iters, thread_id, &timer, manager, perf_counters_measurement);
...
BenchmarkRunner::BenchmarkRunner(const benchmark::internal::BenchmarkInstance &b_,
                                 BenchmarkReporter::PerFamilyRunReports       *reports_for_family_):
    b(b_),
    reports_for_family(reports_for_family_), min_time(!IsZero(b.min_time()) ? b.min_time() : FLAGS_benchmark_min_time),
    min_warmup_time((!IsZero(b.min_time()) && b.min_warmup_time() > 0.0) ? b.min_warmup_time()
    ...
    perf_counters_measurement_ptr(perf_counters_measurement.IsValid() ? &perf_counters_measurement : nullptr) {
    ...
    if (b.aggregation_report_mode() != internal::ARM_Unspecified) {
        ...
        BM_CHECK(FLAGS_benchmark_perf_counters.empty() || perf_counters_measurement.IsValid())
...
bool BenchmarkRunner::ShouldReportIterationResults(const IterationResults &i) const {
    ...
    return i.results.has_error_ || i.iters >= kMaxIterations || // Too many iterations already.
           ...
           ((i.results.real_time_used >= 5 * GetMinTimeToApply()) && !b.use_manual_time());
}

double BenchmarkRunner::GetMinTimeToApply() const {
    // In order to re-use functionality to run and measure benchmarks for
    // running a warmup phase of the benchmark, we need a way of telling whether
    // to apply min_time or min_warmup_time. This function will figure out if we
    // are in the warmup phase and therefore need to apply min_warmup_time or if
    // we already in the benchmarking phase and min_time needs to be applied.
...
void BenchmarkRunner::RunWarmUp() {
    ...
    // benchmark and it would therefore get rid of the need to figure out how
    // many iterations are needed if min_time is set again, this may also be a
    // complete wrong guess since the warmup loops might be considerably slower
    // (e.g because of caching effects).
    ...
    for (;;) {
        ...
        // iterations at once without measuring time isn't as important as it is
        // for the benchmarking phase, we still do it the same way as otherwise
        // it is very confusing for the user to know how to choose a proper
        // value for min_warmup_time if a different approach on running it is
        // used.
...
void BenchmarkRunner::DoOneRepetition() {
    ...
    // After running the warmup phase the BenchmarkRunner should be in a state
    // as this warmup never happened except the fact that warmup_done is set.
    // Every other manipulation of the BenchmarkRunner instance would be a bug!
    // Please fix it.
    if (!warmup_done)
        RunWarmUp();
    ...
    for (;;) {
        ...
        // If we are doing repetitions, and the first repetition was already
        // done, it has calculated the correct iteration time, so we have run
        // that very iteration count just now. No need to calculate anything.
        // Just report. Else, the normal rules apply.
    ...
    if (memory_manager != nullptr) {
        ...
        b.Setup();
        RunInThread(&b, memory_iterations, 0, manager.get(), perf_counters_measurement_ptr);
        manager->WaitForAllThreads();
        manager.reset();
        b.Teardown();
    ...
    BenchmarkReporter::Run report =
        CreateRunReport(b, i.results, memory_iterations, memory_result, i.seconds, num_repetitions_done, repeats);
