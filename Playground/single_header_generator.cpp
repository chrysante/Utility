...
namespace utl {
	...
	static std::string make_single_header_impl(std::filesystem::path const& path, std::unordered_set<std::string>& map) {
		...
		while ((pos = text.find(token, pos)) != std::string::npos) {
			...
			std::string const include_filename = std::filesystem::path(path).replace_filename(text.substr(pos + 1, end - pos - 1));
...
int _ = []{
	...
	auto const single_header_path = (std::filesystem::path(path).remove_filename() / "../mtl_single_header/mtl.hpp").lexically_normal();
