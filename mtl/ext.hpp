...
namespace _VMTL {
	...
	constexpr bool is_normalized(AABB<T, Dim, O> const& r) {
		return r.extend.map([](auto&& x) { return x >= 0; }).fold(mtl::__mtl_logical_and);
	}
	...
	constexpr T volume(AABB<T, Dim, O> const& r) {
		return fold(map(r.extend - r.origin, abs), mtl::__mtl_multiplies);
	}
	...
	constexpr bool do_intersect(AABB<T, Dim, O> a, AABB<U, Dim, P> b) {
		...
		return map(a.origin,
				   ...
				   [](auto aMin,
					  auto aMax,
					  auto bMin,
					  auto bMax) { return aMin <= bMax && aMax >= bMin; }).fold(mtl::__mtl_logical_and);
	}
